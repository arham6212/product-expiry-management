-- Shop selling price is Product metadata. Keep it nullable so historical
-- Products remain valid, while the receiving RPC requires it for new stock.
alter table public.products
add column selling_price_minor integer
  check (selling_price_minor is null or selling_price_minor between 1 and 2147483647);

drop function public.receive_product_stock(uuid, uuid, integer, date, text, text);
