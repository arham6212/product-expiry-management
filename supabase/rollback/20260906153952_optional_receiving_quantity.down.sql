-- Refuse rollback while unknown quantities exist; never invent counts.
begin;
do $$ begin
  if exists(select 1 from public.batches where current_quantity is null)
    or exists(select 1 from public.inventory_movements where quantity_delta is null) then
    raise exception 'Unknown quantities exist; rollback requires an explicit audited resolution.';
  end if;
end $$;
alter table public.batches alter column current_quantity set not null;
alter table public.inventory_movements alter column quantity_delta set not null;
alter table public.inventory_movements drop constraint inventory_movements_unknown_received_only;
