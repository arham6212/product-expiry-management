-- NULL means unknown, never a fabricated zero/one. Existing rows are untouched.
alter table public.batches alter column current_quantity drop not null;
alter table public.inventory_movements alter column quantity_delta drop not null;
-- A null delta is allowed only for a received observation; other operations
-- retain their existing required quantity and direction.
alter table public.inventory_movements add constraint inventory_movements_unknown_received_only
  check (quantity_delta is not null or movement_type = 'received');
...
create or replace function public.receive_product_stock(
  ...
  target_selling_price_minor integer
)
returns table (
  batch_id uuid,
  batch_shop_id uuid,
...
begin
  ...
  if received_quantity is not null and received_quantity <= 0 then
  ...
  if target_selling_price_minor is null or target_selling_price_minor <= 0 then
  ...
  set selling_price_minor = target_selling_price_minor
...
revoke all on function public.receive_product_stock(
  uuid, uuid, integer, date, text, text, integer
) from public, anon, authenticated;
grant execute on function public.receive_product_stock(
  uuid, uuid, integer, date, text, text, integer
) to authenticated;

create or replace function public.get_expiry_dashboard(target_shop_id uuid)
returns table (
  reference_date date,
  shop_id uuid,
  batch_id uuid,
...
begin
  ...
  if not exists (
    ...
    where batch.shop_id = target_shop_id
      and (batch.current_quantity is null or batch.current_quantity > 0)
  ) then
    return query
    select
      shop_reference_date,
